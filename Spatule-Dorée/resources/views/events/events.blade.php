...
@section('content')
    ...
    //dd($events);
    ?>
    <link href="{{ asset('css/events.css') }}" rel="stylesheet" type="text/css">
                    ...
                    <div class="padding-60px-lr md-padding-50px-lr sm-padding-30px-all xs-padding-25px-all">
                        ...
                        <ul class="event-time margin-10px-bottom md-margin-5px-bottom">
                            <li><i class="far fa-clock margin-10px-right"></i> {{ $event->start_event }} -
                                {{ $event->end_event }}</li>
                            <li><i
                                    class="fas fa-user margin-5px-right"></i>{{ isset($event['users']) && count($event['users']) > 0 ? $event['users'][0]['firstname'] . ' ' . $event['users'][0]['name'] : 'pas de teachers' }}
